...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "db-BM" do |db|
   db.vm.box = "centos/7"
   db.vm.hostname = "db-BM"
   db.vm.network "private_network", ip: "192.168.33.100"
   db.vm.provision "shell", inline: "echo Iam DB server"
  end

  config.vm.define "lb-BM" do |lb|
    lb.vm.box = "centos/7"
    lb.vm.hostname = "lb-BM"
    lb.vm.network "private_network", ip: "192.168.33.200"
    lb.vm.provider "virtualbox" do |vb|
     vb.customize ["modifyvm", :id, "--memory", "512", "--cpus", "1", "--name", "lb-BM"]
    end
    lb.vm.provision "ansible" do |ansible|
      ansible.playbook = "playbooks/haproxy/loadbalancer.yml"
      ansible.extra_vars = {
         "web_servers" => [
          {"name": "web-1-BM","ip":"192.168.33.11"},
          {"name": "web-2-BM","ip":"192.168.33.12"}
         ] 
    }
